...
**申请权限变更**：仅限为当前用户申请变更【进入主站、使用专栏、自由发言、发送私信、使用图床】权限。申请需提供完整理由，由管理员根据[社区规则](../community)处理。**7天内仅能创建一次该类型工单（含撤销或授予权限）。**

:::tip

- 关于【自由发言】权限：自由发言权限被封禁后无法发表公开的工单，但依然可以提交用户工单。
- 对于被封禁的用户（被撤销进入主站权限），可上传手写检讨，申请解封，具体见[社区规则-处罚解除](../community#penalty-revoke)。
