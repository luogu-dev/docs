...
要在洛谷讨论区回复中提及其他用户（即“at”该用户），需要先打出符号 @，紧接着输入完整用户名，等待出现提示后按 Enter 选中。

![at 他人的示例](_image/atExample.png)
...
（请注意输入“`@kkksc03`”后需要等待出现候选项后按 Enter 选中，如果没有出现/不小心关闭可以使用 Ctrl + Space 唤起）

显示的效果如下：

洛谷网的站长是 @[kkksc03](luogu://user/1)，网站于 2013 年开始运营。
...
**在犇犇中，并没有上面的补全机制，请直接打出 `@kkksc03` 并在后方跟上一个空格。**
