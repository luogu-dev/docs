...
洛谷的 Markdown 解析器在基础语法上遵循 [CommonMark 行内解析规则](https://spec.commonmark.org/current/#inlines)，并支持 GFM 与下文说明的洛谷扩展语法。为保证安全性，原始 HTML 标签不会被解析，请使用 Markdown 或本手册提供的扩展语法实现排版。
...
在行末输入**两个空格**或一个反斜线（`\`），可以达到比分段更加紧凑的换行效果。
...
Markdown 使用星号（`*`）和底线（`_`）标记需要强调的文字：一对标记表示*斜体*，两个连续的标记表示**粗体**；两个波浪号（`~~`）可表示~~删除线~~。标记需要紧贴被修饰的内容，不能在标记与内容之间插入空白。
...
星号和底线并非总会按最近的一对自动配对。由 `*`、`_` 形成的斜体和粗体标记，都必须位于可识别的边界；例如结束标记前是标点、后面又紧接文字时，可能无法被识别为强调结束标记。
...
如果 `*` 或 `_` 的两边都有空白，它们会被当作普通符号。若需要在文字前后直接显示星号或底线，可以用反斜线（`\`）转义（参见下面的“反斜杠”部分）。
...
Markdown 建立代码块的方法：在代码的首行和末行分别使用三个或更多的反引号（\`\`\`）或波浪号（`~~~`），并独立成行。首行标记后可加语言名称，例如 `cpp`、`c++`、`java`、`c`、`pascal` 等；未指定语言时，代码会按 C++ 显示。
...
```markdown
[行内式链接标题](https://www.luogu.com.cn/ "可选标题")
```

显示效果为：

这是[行内式链接标题](https://www.luogu.com.cn/ "可选标题")。可将光标移至链接上查看 title 效果。
...
在正常 Markdown 语法的基础上，结合 `^` 向上合并单元格，结合 `<` 向左合并单元格。在同一个表格内可混合使用这两种方式。合并标记必须是单元格内唯一的纯文本内容，不能与其他文字或 Markdown 格式混用。
...
![折叠框示例](./_image/callout.png)

嵌套语法：最深的一层是三个 `:`，每往外一层就增加一个 `:`。区块引用、列表和折叠框等容器的支持组合嵌套，请合理使用。
...
可用图片语法插入 Bilibili 视频。

具体效果见 [https://www.luogu.com.cn/paste/okqcziz9](https://www.luogu.com.cn/paste/okqcziz9)。
