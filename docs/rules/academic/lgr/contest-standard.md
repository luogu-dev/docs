...
- 负责人：原则上要求 8 级及以上（金色）奖项认证用户作为比赛负责人，但对于小月赛，7 级奖项认证用户也可以。
- 出题履历（可选）：如果负责人只有 7 级奖项认证，强烈建议包含此项。
...
- 明确题目来源，包括**出题人资格**和 **idea 形成的过程**，其中**出题人原则上要求 6 级及以上（蓝色和金色）奖项认证用户**，非 6 级及以上（蓝色和金色）奖项认证用户的出题人需提供出题履历并由月赛审核员额外审查。
- 明确题目质量和知识点分布。
...
- 负责人应当在赛后至多一小时在**学术版**发布**赛后总结帖**，内容包括**各题的通过率**（通过率计算为该题满分人数除以整场比赛非 0 分的人数）、一血（可选）、**赛时出现的所有失误及致歉**（没有锅当然就可以不致歉）和奖品发放的结果（可选）等。**该帖同时视为官方的评价渠道。**
...
- 该比赛有额外的难度要求：
  - 基础赛至少要有四道题
  - **第一题**必须是【入门】难度。正常写标程代码不超过 300B。涉及到数学必须是小学数学的范围内，不能超过 GESP 大纲三级，基准通过率 80%。
...
- 该比赛有 1000 元的基准出题费，详见[细则](./payroll.md)。
