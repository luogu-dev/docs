...
请注意，在线 IDE 模式的时空限制与正式提交评测的时空限制不同，且仅支持一定数量的输入数据，不支持文件读写等高级交互评测形式，正式评测的时空限制请参考题目详情页面的“时间限制”和“内存限制”。
...
如果真的觉得自己的题解非常好，或者之前的题解大多有疏漏，那么可以在工单系统中的[申请全站推荐](https://www.luogu.com.cn/ticket?page=1&type=article.promote)提出工单申请。如果确认为特别具有价值的优质题解，可以特许进入题解区。

若低质量题解多次申请，会被视为扰乱工单处理秩序，可能会被予以警告或禁言的处罚。
...
- 数据可能真的有问题。但是，如果已有不止一个人通过了这道题，那最好不要怀疑是数据存在问题。
