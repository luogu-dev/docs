...
- 实名认证要求：洛谷的一部分社区功能要求用户进行实名认证（即绑定手机号或完成实人认证），才能使用。
...
- 为什么被提示“您已被禁言或非受信任的用户，无法回复”：请参考[为什么被提示“您已被禁言或非受信任的用户，无法回复”](#cannot-reply)。

## 洛谷用户权限体系 {#user-permissions}

洛谷对于用户的功能使用权限由一系列规则确定。可大概分为以下几个等级：

1.仅绑定邮箱：可以使用网站的基本功能，可登录后查看题目、提交代码进行评测、查看讨论等。基本满足训练学习需求。
2.绑定手机号或完成实人认证：可以使用需要实名认证的功能如发布讨论、回复讨论、提交题解/文章、提交部分工单等。
3.成为受信任的用户：满足任意一个条件，即已认证的奖项不少于一个，或比赛等级分不低于 1000，或用户名颜色为橙名及以上。可获得更高的用户权限，可在学术版发帖回复、回复题目对应讨论版帖子、举报其他用户等。
...
新用户的初始动态等级为 0，也就是灰色用户名。用户在洛谷中按照洛谷社区规则使用各项功能，会增加动态等级。如果长时间不使用洛谷或者有违规行为，可能会降级。除非用户有严重违规行为或者在长时间不使用洛谷且未进行实名认证的情况下，一旦用户达到蓝名，则不会掉到灰名。你可以在打卡之后在打卡结果中看到自己用户名的颜色。**在工单、讨论区或者犇犇中询问颜色名字和咕值相关的一些问题将被和谐。**

如果用户抄袭题解或非恶意比赛作弊，将清空所有 AC 记录、展示为棕名，并在用户名旁展示“作弊者”标签。标签将持续展示 15 日，并且在 90 日内参与任何比赛都不会获得排名。15 日后，扣除相应咕值并清空所有 AC 记录。（注：**棕名用户是仅次于封号的最严厉警告，棕名后如果发生任何违反洛谷社区规则的事情，将立刻封禁一年。**）

动态等级影响洛谷内很多权限，例如创建比赛、团队、提交冷却时间、图床、下载数据等。灰名会有较多的限制。

## 为什么被提示“您已被禁言或非受信任的用户，无法回复” {#cannot-reply}

收到此提示意味着您当前无法进行回复操作，通常是因为以下两种情况之一：

### 1. 账号处于“禁言”状态

如果您因违反社区规则（如发布无意义内容、违规言论等）被管理员处罚，在禁言期间将无法在任何板块发言。
...
洛谷的部分板块（如**站务版**、**学术版**）以及**题目讨论帖**（非楼主回复时）启用了质量管控，限制仅“受信任的用户”可以回复。

- **如何成为“受信任的用户”？**（满足以下**任意一个**条件即可，系统会自动判断）：
  1. **奖项认证**：已认证的奖项不少于一个（[去认证](https://www.luogu.com.cn/user/setting/prize)）。
  2. **比赛等级分**：不低于 1000 分。
  3. **用户等级**：用户名颜色为**橙名**及以上（通过持续刷题和参加比赛提升活跃度）。
- **建议**：如果您暂时未达标，建议先专注于提升自己的做题数量和比赛成绩，达成上述任一条件后即自动解锁相应权限。
