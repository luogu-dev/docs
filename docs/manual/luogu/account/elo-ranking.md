...
开放比赛等级分计算的比赛会在比赛列表的比赛名称后显示金色的奖杯图标，比赛详情的右侧边栏也会有详细说明。点击【报名】按钮时会确认本场是否参与等级分计算。

仅限咕值计算的比赛在比赛列表仅显示绿色奖杯图标。点击【报名】按钮时会直接显示确认公开赛参与规则。

![比赛列表](./_image/contest-list.png)

![报名比赛](./_image/contest-join-elo.png)

![比赛详情](./_image/contest-detail.png)

对于开放比赛等级分计算的比赛，每场比赛中，都设有一个阈值和初始值。对于没有等级分评级历史的，会默认等级分为这个初始值并进行计算。当选手的等级分超过阈值时，将不再获得评级。对于多数比赛，阈值设定为：
...
对于部分比赛，根据实际情况，也可能设置为其他合适的数字。用户报名比赛时，也可以选择不参与比赛等级分评定。

首次参加比赛时由于没有更早的胜率数据，而又排名靠后时，会因为评分达不到阈值而记为 0 分。一般来说，大部分选手会在第二场比赛过后就有等级分。

比赛等级分会在赛后的一周内更新，请勿催促更新。
