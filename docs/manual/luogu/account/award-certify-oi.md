...
非以上奖项无法在洛谷获得认证（包括 CSP 第一轮），以上奖项以 **CCF 官方公布名单**（[https://www.noi.cn/hjmd/mdcx/](https://www.noi.cn/hjmd/mdcx/)）为准，与是否领取到了纸质证书无关。目前 CCF 不公布初中组的获奖信息，没有一个统一的可信数据源进行参考，因此没法单独进行认证，更没法计算积分。

洛谷使用的数据来自 [OIerDB](https://oier.baoshuo.dev/)。当名单公布后，因报名时的学校名称十分自由，初期会有较多的学校名称修正（多见于 CSP-J/S），故洛谷**不会即时进行同步**，请不要催促。
...
2. 选择认证类型。
3. 选择自己获得过的奖项。（系统会根据当前用户 IP 地址和账号的实人认证信息匹配所有同地域同名的奖项，请**诚实地**选择自己的奖项。）
...
用户展示的奖项会在每次更新咕值时影响获得成就分的计算。咕值非实时更新，具体见[咕值更新](guzhi.md#guzhi-update)。

##### 自动关联新奖项以及再次认证

当新奖项导入后（以洛谷公示的“[支持认证的比赛](https://www.luogu.com.cn/offline-prize/supported-events)”为准），洛谷将自动追加同名同校的新增奖项以及对部分同地域的升学情形进行自动匹配。

若未能自动更新，您可尝试在奖项认证页面手动选择自己的奖项。
...
1. 在个人中心中显示自己曾经获得的奖项，用户名旁边根据奖项显示已认证标记。并根据积分规则计算等级并展示（3-5 级为绿色，6-7 级为蓝色，8 级及以上为金色），以增加社区活动的学术可信度。
