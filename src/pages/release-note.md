...
- 整体重写：基于 VUE3 新前端框架重写个人设置页面。
- 奖项认证流程更新：拆分支付宝实人认证至安全设置页面（新接口不再要求本人支付宝）；奖项认证奖自动匹配所有可能奖项供用户选择，不在要求单次只能选择一所学校，大大提高自动认证成功率。
- 偏好设置里增加了**代码字体**选项，您可以自行设置您的偏好字体。如果希望使用浏览器默认字体，请您搜索“如何给 XX 浏览器设置默认字体”，并自行修改浏览器字体。

### 题解提交限制

增加题解提交门槛，原则上必须有该题的通过记录才允许提交该题的题解（RMJ 题除外）。
